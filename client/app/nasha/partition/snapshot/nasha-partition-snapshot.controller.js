angular.module("managerApp").controller("NashaPartitionSnapshotCtrl", function ($stateParams, $scope, $uibModalInstance, $q, $translate, DedicatedNasha, Toast) {
    "use strict";
    var self = this;
    ...
    self.getAddSchedulesPromises = function (promises) {
        ...
        _.forEach(addToSchedule, function (schedule) {
            promises.push(DedicatedNasha.Partition().Snapshot().Lexi().add({
    ...
    self.getDeleteSchedulesPromises = function (promises) {
        ...
        _.forEach(deleteFromSchedule, function (schedule) {
            promises.push(DedicatedNasha.Partition().Snapshot().Lexi().remove({
                serviceName: self.data.nashaId,
                partitionName: self.data.partition.partitionName,
                snapshotType: schedule
            }).$promise.then(function (result) {
    ...
    function init () {
        self.loading = true;
        DedicatedNasha.Lexi().schema()
            .$promise.then(function (data) {
                self.snapshotEnum = data.models["dedicated.storage.SnapshotEnum"].enum;
            });

        DedicatedNasha.Partition().Snapshot().Lexi().query({
            serviceName: self.data.nashaId,
            partitionName: self.data.partition.partitionName
        }).$promise.then(function (data) {
