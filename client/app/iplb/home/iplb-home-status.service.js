class IpLoadBalancerHomeStatusService {
    ...
    getIPLBStatus (serviceName, config = { toArray: false }) {
        return this.OvhApiIpLoadBalancing.v6().status({ serviceName })
            ...
            .then(response => {
                ...
                const promises = _.map(_.keys(_.pick(response, _.keys(this.iplbItemWeight))), iplbItem => {
                    response[iplbItem].itemName = iplbItem;
                    return this._transformIplbItem(serviceName, response[iplbItem]).then(item => {
    ...
    _transformIplbItem (serviceName, iplbItem) {
        switch (iplbItem.itemName) {
            case "service":
                return this._buildServiceStatusItem(serviceName, iplbItem);
            default:
                return this._buildOtherStatusItem(iplbItem);
        }
    }
    ...
    _buildOtherStatusItem (iplbItem) {
