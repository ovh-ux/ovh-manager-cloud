class IpLoadBalancerVrackService {
    constructor ($q, IpLoadBalancerServerFarmService, OvhApiIpLoadBalancing, IpLoadBalancerTaskService, OvhApiVrack,
    ...
    addPrivateNetwork (serviceName, network) {
        return this.OvhApiIpLoadBalancing.Vrack().v6().post({ serviceName }, _.omit(network, ["vrackNetworkId", "farmId"]))
            .$promise
            .then(response => this.OvhApiIpLoadBalancing.Vrack().v6().updateFarmId({ serviceName, vrackNetworkId: response.vrackNetworkId }, { farmId: network.farmId }).$promise)
            .then(response => {
    ...
    editPrivateNetwork (serviceName, network) {
        return this.$q.all([
            ...
            this.OvhApiIpLoadBalancing.Vrack().v6().updateFarmId({ serviceName, vrackNetworkId: network.vrackNetworkId }, { farmId: network.farmId }).$promise
        ])
            .then(response => {
                this.OvhApiIpLoadBalancing.Farm().v6().resetQueryCache();
                this.OvhApiIpLoadBalancing.Farm().Tcp().v6().resetCache();
    ...
    deletePrivateNetwork (serviceName, networkId) {
        return this.OvhApiIpLoadBalancing.Vrack().v6().updateFarmId({ serviceName, vrackNetworkId: networkId }, { farmId: [] }).$promise
            .then(() => this.OvhApiIpLoadBalancing.Vrack().v6().delete({ serviceName, vrackNetworkId: networkId }).$promise)
