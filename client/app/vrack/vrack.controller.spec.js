describe('Controller: VrackCtrl', () => {
  let $httpBackend;
  let $controller;
  let scope;
  let $rootScope;
  let CloudMessageMock;
  let $translateMock;
  let $stateMock;

  let servicesRequestHandle;
  let allowedServicesRequestHandle;
  const dataTestServices = {
    ip: ['', ''], dedicatedServer: ['', ''], dedicatedCloud: ['', ''], legacyVrack: ['', ''], cloudProject: ['test', ''],
  };
  const dataTestAllowedServices = {
    ip: ['', ''], dedicatedServer: ['', ''], dedicatedCloud: ['', ''], legacyVrack: ['', ''], cloudProject: ['test', ''],
  };
  ...
  function getNumberOfService(services) {
    const serviceTypes = ['dedicatedServer', 'dedicatedCloud', 'legacyVrack', 'ip', 'cloudProject'];
  ...
  describe('Test func getDisplayName.', () => {
    it('Get the display name for a specific type.', () => {
  ...
  describe('Test the functions that get the services.', () => {
    xit('Get all the services allowed.', () => {
      controller.getAllowedServices().then((services) => {
        expect(getNumberOfService(services)).toBe(12);
      });
    });
    it('Get all the services for a specific vrack.', () => {
      controller.getVrackServices().then((services) => {
        expect(getNumberOfService(services)).toBe(10);
      });
    });
  });

  describe('Test error handling (CloudMessage)', () => {
    it('addSelectedServices error.', () => {
    ...
    it('deleteSelectedServices error.', () => {
